...
public class HarmonicRotation : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        Vector3 mod_time = new Vector3(((periodRatio.x*elapsedTime)%twoPI)-PItwo, ((periodRatio.y*elapsedTime)%twoPI)-PItwo, ((periodRatio.z*elapsedTime)%twoPI)-PItwo);        
        ...
        Vector3 delta = (ampPos-lastPos);
        Vector3 current = transform.rotation.eulerAngles;
        transform.rotation = Quaternion.Euler(current.x+delta.x, current.y+delta.y, current.z+delta.z);
        lastPos = ampPos;
    }
}
