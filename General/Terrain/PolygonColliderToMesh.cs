...
public class PolygonColliderToMesh : MonoBehaviour {
    public Vector2 meshTextureScale;
    ...
    public void Reshape() {
        ...
        //have to loop because vector2[] cannot be cast to vector3[] 
        ...
        //MY ADDITION:;
        //modified uvs so we can alter texture size
        Vector2[] modUvs = new Vector2[_cachedPoints.Length];
        for (int i = 0; i < vertices.Length; i++){
            //modUvs[i] = new Vector2((meshVertices[i].x/length)*xRepeat, (meshVertices[i].y/height)*yRepeat);
            //modUvs[i] = (vertices[i]/boundsSize)*repeat;
            modUvs[i] = (vertices[i] / meshTextureScale);
        }
        ...
        //honestly this is the stupidest thing I've ever seen  \/
        //nevermind
